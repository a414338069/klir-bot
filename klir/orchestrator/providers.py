...
class ProviderManager:
    ...
    def resolve_runtime_target(self, requested_model: str | None = None) -> tuple[str, str]:
        ...
        # When config explicitly sets opencode_http, override model-based routing
        # (OpenCode HTTP uses the same provider/model format as CLI OpenCode,
        # so model-based routing alone cannot distinguish them.)
        if self._config.provider == "opencode_http" and provider == "opencode":
